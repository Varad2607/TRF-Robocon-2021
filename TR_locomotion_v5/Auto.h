...
class Auto: public Move {
  ...
  public:
    ...
    */
    ...
    Auto(MPU6050 *mpu, Encoder *encX, Encoder *encY, Motor *motor1, Motor *motor2, Motor *motor3): Move(mpu, encX, encY, motor1, motor2, motor3) {
    ...
    */
    ...
    void auto_left(float kp1, float kd1, float ki1, float kp2, float kd2, float ki2, float kp3, float kd3, float ki3, float kp_enc_y_left, float kd_enc_y_left, float ki_enc_y_left, float kp_enc_auto_left, float kd_enc_auto_left, float ki_enc_auto_left, float distance)
    {
      ...
      while (encX_move->currentDistance < distance) {
        ...
        if (abs(encX_move->error_distance) > 5) {
    ...
    */
...

    {
      ...
      while (encX_move->currentDistance < distance) {
        ...
        if (abs(encX_move->error_distance) > 10) {
   ...
   void auto_left_seq(float dist1, float dist2, float dist3){
    if(left_flag == 1){
        ...
        auto_left(1.1, 0, 0, 1, 0, 0, 1, 0, 0, 0.03, 0, 0, 2.4, 0, 0, dist2);
        left_flag = 3;
      }

      else if(left_flag == 3){
        auto_left(1.1, 0, 0, 1, 0, 0, 1, 0, 0, 0.03, 0, 0, 2.4, 0, 0, dist3);
        left_flag = 1;
      }
   }
