...
class PFR
{
 ...
 //-------------------------------------------------------------------------------------------------------------------------------------------
  ...
  void readError(int pin){
    pot_error = analogRead(pin);
    pot_error_ = map(pot_error, 11, 238, 0, 1000);
    Serial.println(pot_error_);  
  }
 ...
 //-------------------------------------------------------------------------------------------------------------------------------------------
    ...
    */
    ...
    void pick_from_rack(int arrows, int kp, int kd, int ki)
    {
      ...
      //pwm = base_rotate_pwm + (kp * pot_error + kd * (pot_d_error) + ki * (pot_i_error));
